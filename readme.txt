It's recommended to quickly read README.md, as that was the original readme file included with the tool. Specifically note the requirements section, as they are the same here.

If the root folder (the folder this .txt file is in) has moved or has been renamed, delete the venv folder if it exists and follow first time installation instructions.

FOR FIRST TIME INSTALLATION:

1. Open root folder (folder with this .txt file) in cmd and run "python -m venv venv" without quotes
...
1. In the folder this .txt file is in, navigate to venv\Scripts\
